...
- LLMカード生成
- AIカードJSON schema検証
- active / pinned カードの固定上限なし表示と右ペイン内スクロール
...
| UI-003 | `owner` または `user` がserver-controlled app metadataで明示された認証済み利用者は、ログイン後にSession Setupへ遷移する。 |
| UI-004 | 認証・権限情報以外の会話データをAuth DBへ保存しない。 |
| UI-023 | 利用者は現在のブラウザセッションからログアウトできる。ログアウトだけではユーザー別ローカル保存を削除しない。 |
...
| UI-011 | `セッション開始` 操作により、文字起こし + AI補助カード画面へ遷移し、選択済み音声ソースへの接続を自動開始する。 |
...
| UI-024 | 利用者向け画面ではoperator用の診断ボタンとダミー文字起こし開始ボタンを表示しない。 |
...
| UI-021 | ユーザー明示操作により、設定・final transcript・AIカード・Session Reportをブラウザへローカル保存できる。音声ファイルは対象外とする。 |
| UI-022 | Session Setupで保存済みセッションを一覧表示し、再表示・削除できる。 |
...
| 状態 | 意味 |
| --- | --- |
| `active` | 右ペインに表示中。固定件数上限はなく、カード一覧内のスクロールで全件へ到達可能 |
| `queued` | 処理中または旧状態との互換用の表示待ち。評価時にactiveへ昇格する |
| `later` | あとで回収 |
| `done` | 聞いた / 回収済み |
| `dismissed` | 不要として抑制 |
| `pinned` | 固定。自動降格しない |
...
1. active / pinned カードには固定件数上限を設けない。
2. 新規候補はスコアリングと `dedupeKey` による重複判定を通す。
3. 重複していない新規候補はactiveへ追加する。
4. 表示件数を理由に、既存カードを自動降格・入れ替えしない。
5. `pinned` は固定表示を維持する。
6. `done` / `dismissed` は同一論点の再表示を抑制する。
7. `再判定` では active / queued / later を再評価する。
8. 右ペインの高さを保ち、カード一覧内のスクロールで全active / pinnedカードへ到達できるようにする。
...
| ローカル保存データ | ユーザーIDで分離したBrowser localStorage | ユーザーが一覧またはブラウザのサイトデータから削除するまで |
...
| NFR-SYS-002 | 通常のLLM文脈レビューは40秒に1回以下、高重要度時も16秒のクールダウン後を目標にする。 |
| NFR-SYS-003 | active / pinnedカードには固定表示上限を設けず、右ペイン内スクロールで全件へ到達可能にする。 |
...
- 7件以上のactive / pinnedカードも保持され、右ペイン内スクロールで全件を確認できる。
- 新規カードが来ても、表示件数を理由に既存カードがqueuedへ移動しない。
...
- 保存後に画面を再読み込みしても、Session Setupの保存済み一覧からセッションを再表示・削除できる。
