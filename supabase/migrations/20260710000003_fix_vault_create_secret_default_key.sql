-- Let Vault select its managed default encryption key. Passing an explicit NULL
-- key_id bypasses that default in some Supabase Vault versions and causes a
-- NOT NULL violation inside vault.secrets.
...
begin
  ...
  if found then
    perform vault.update_secret(v_secret_id, v_api_key, null, null);
    ...
    select vault.create_secret(v_api_key, null, null) into v_secret_id;
